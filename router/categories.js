const {Category} = require("../models/category");
const express = require("express");
const router = express.Router();

//GETTING A PROFUCT
router.get(`/`, async (req, res)=>{
    const categoryList = await Category.find();

    if(!categoryList) {
        res.status(500).json({success: false})
    }
    res.status(200).send(categoryList);
})

//GETTING A PRODUCT BY ID FROM A CERTAIN CATEGORY
router.get("/:id", async (req, res) => {
    const category = await Category.findById(req.params.id);
    if(!category) {
        res.status(500).json({message: "The category you are looking for does not exist"})
    }
    res.status(200).send(category);
})

//POSTING A NEW PRODUCT
router.post("/", async (req, res) => {     //req gets info from the front end and the res is used to read them
    let category = new Category({
        name: req.body.name,
        icon: req.body.icon,
        colour: req.body.colour
    })
    category = await category.save();

    if(!category)
    return res.status(404).send("The category cannot be created")

    res.send(category);
})

//put is used to update the categories. UPDATING THE CATEGORIES
router.put("/:id", async (req, res) => {
    const category = await Category.findByIdAndUpdate(
        req.params.id, 
        {
            name: req.body.name,
            icon: req.body.icon,
            colour: req.body.colour
        },
        { new: true }
    )
    if(!category)
    return res.status(404).send("The category cannot be created")

    res.send(category);
})

// api/v1/ "+id" DELETING A PRODUCT
router.delete("/:id", (req, res) =>{
    Category.findByIdAndRemove(req.params.id).then(category => {
        if(category) {
            return res.status(200).json({success:true, message: "Category is deleted"})
        }else{
            return res.status(404).json({success:false, message: "You are fucked! That category does not exist"})
        }
    }).catch(err => {
        return res.status(400).json({success: false, error: err})
    })
})

module.exports = router;